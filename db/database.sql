...
CREATE TABLE `user` (
  `user_ID` int(3) ZEROFILL NOT NULL AUTO_INCREMENT,
  `user_Name` varchar(15) NOT NULL,
  `user_Password` varchar(100) NOT NULL,
  `user_Theme` varchar(50) NOT NULL DEFAULT "default",
  PRIMARY KEY (`user_ID`),
...
CREATE TABLE `task` (
  `task_ID` int(4) ZEROFILL NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `subject` (
  ...
  `subject_Icon` varchar(1) DEFAULT NULL, -- show an emoji picker when adding or editing a subject
...
CREATE TABLE `note` (
  `note_ID` int(2) ZEROFILL NOT NULL AUTO_INCREMENT,
