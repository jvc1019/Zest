<div id="notes_tag">
    ...
    <?php
        $tags = !empty($search_tag) ? $search_tag : $keyword;
        $query = "SELECT * FROM note $tags";
        $result = $conn->query($query);
        if (!($result->num_rows > 0)) {
            echo "<h6 class='text-center'>No results found.</h6>";
        } else {
            if (!empty($keyword)) { ?>
        <h6><a href="notes.php" class="btn btn-sm" data-toggle="tooltip" title="Clear tag">&times;</a>Tagged notes
